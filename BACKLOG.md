...
activeDate **2026-07-03**). (Identifier note: this is EXC §314, NOT State Finance Law.)

RECONCILIATION (2026-07-12): the divergence was a **benign version-transition, NOT a
substantive amendment.** The stored copy carried a DUAL-VERSION subd. 5 (the
pre-2026-07-01 single-paragraph "5." + `NB Effective until July 1, 2026`, plus the
operative `5. (a)/(b)/(c)`); the 2026-07-03 revision dropped the expired paragraph and
both `NB Effective ...` notes, printing only the operative version. Human-verified
against the official nysenate.gov section page (text pulled in-browser, egress-blocked
in-sandbox). **The five-year validity in 5(a) is UNCHANGED; the repeal date
(2028-07-01) is UNCHANGED. No rule change.** The 5 NYCRR 144 clause in 5(b) was ALREADY
present in the stored capture (activeDate 2026-02-20) — it was NOT the delta.

TEXT RE-CAPTURE — staged (draft PR, held for SHA review): STATE TEXT re-captured to the
single-version live text, verbatim; full-section per-subdivision **bounded** normalized
diff verifies stored == live for every subdivision (whole-section 10136 == 10136 bytes,
subd 5(b) 1012 == 1012). Metadata + VERSION HISTORY annotation updated. NO engine/logic
change. This clears the *text* divergence.

FRESHNESS FLIP — SEPARATE, post-approval step (different gate): §314 stays
NEEDS_REVIEW / fail-closed until the DIVERGENT verdict is cleared. The verdict lives in
`data/config/freshness-state.json` (+ the dated report), NOT in the golden copy, and is
regenerated by a real freshness run — do NOT hand-write it as part of the text-sync.

REMAINING OPEN ITEM — 5 NYCRR Part 144 (two-part; attorney gate INTACT):
§314(5)(b) creates a recertification rebuttable presumption conditioned on the
application being "compliant with **5 NYCRR 144**" — a regulation NOT captured as a
golden source. Handling is a self-attestation feature (show the vendor the verbatim
rule; the vendor confirms compliance; the engine never asserts it). That does NOT
retire the attorney item — selection of which rules govern, and the legal weight of
soliciting/storing a compliance attestation in a state MWBE context (§314(1) references
fraudulent-misrepresentation revocation), are licensed-review questions that neutral
verbatim wording does not dissolve.
  - **(a) CAPTURE — no lawyer:** pull 5 NYCRR Part 144 verbatim from the official
    source, human-verify side-by-side. Gates everything downstream (can't design the
    display before reading 144). Blocked in-sandbox on egress to the NYCRR source
    (govt.westlaw.com / dos.ny.gov); unblock via a real run or a human paste.
  - **(b) LICENSED REVIEW — required, NOT optional:** given the captured 144 text, an
    attorney confirms whether verbatim-display + self-attestation is sound handling of
    the 5(b) presumption, or whether soliciting that attestation creates exposure —
    and how it should be worded/stored. The L-grade on §314(5)(b) keeps the engine
    fail-closed (surfaces only `RECERT_PRESUMPTION_PENDING`, never asserts) while open.

Significance: the FIRST real divergence the freshness system caught in production — the
